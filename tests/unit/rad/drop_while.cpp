...
inline constexpr auto fn = [](size_t i)
{
    return i < 3;
};
...
TEST(drop, input)
{
    auto ra = radr::test::iota_input_range(1, 7) | radr::drop_while(fn);
...
struct drop_while_forward : public testing::Test
{
    /* data members */
...
TYPED_TEST_SUITE(drop_while_forward, container_types);

TYPED_TEST(drop_while_forward, rvalue)
{
    auto ra = std::move(this->in) | radr::drop_while(fn);
...
TYPED_TEST(drop_while_forward, lvalue)
{
    auto ra = std::ref(this->in) | radr::drop_while(fn);
...
TYPED_TEST(drop_while_forward, folding)
{
    auto fn1 = [](size_t i)
    {
        return i < 2;
    };
    auto ra = std::ref(this->in) | radr::drop_while(fn1) | radr::drop_while(fn);
