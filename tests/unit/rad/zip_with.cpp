...
TEST(zip_with, SingleRange)
{
    std::vector<int> vec{1, 2, 3};
    auto             z = radr::zip_with()(std::ref(vec));
...
{
    auto z = std::vector<int>{10, 20, 30} | radr::zip_with();
...
TEST(zip_with, MixedRvalueAndRef)
{
    ...
    auto              z = std::vector<int>{1, 2, 3} | radr::zip_with(std::ref(chars));
...
TEST(zip_with, ReferencesAreMutable)
{
    std::vector<int> a{1, 2, 3};
...
TEST(zip_with, ConstReferencesNotMutable)
{
    ...
    auto z = std::cref(a) | radr::zip_with(std::cref(b));
...
TEST(zip_with, ConstReferencesNotMutable2)
{
    std::vector<int> a{1, 2, 3};
    std::vector<int> b{4, 5, 6};

    auto const z = std::ref(a) | radr::zip_with(std::ref(b));
...
TEST(zip_with, ReferenceTypesAreTuples)
{
    ...
    auto z = std::ref(xs) | radr::zip_with(std::ref(ys));
...
TEST(zip_with, MoveOnlyElements)
{
    ...
    auto z = std::ref(ptrs) | radr::zip_with(std::ref(multipliers));
...
TEST(zip_with, VectorAndArray)
{
    std::vector<int>    vec{1, 2, 3};
    std::array<char, 3> arr{'a', 'b', 'c'};

    auto z = std::ref(vec) | radr::zip_with(std::ref(arr));
...
TEST(zip_with, ListAndDeque)
{
    ...
    auto z = std::ref(lst) | radr::zip_with(std::ref(dq));
...
TEST(zip_with, BorrowedRanges)
{
    ...
    auto z = std::ref(vec) | radr::zip_with(std::ref(sv));
...
TEST(zip_with, WithRadrRanges)
{
    ...
    auto z = std::move(iota_rng) | radr::zip_with(std::ref(chars));
...
TEST(zip_with, ConceptsForRandomAccessRanges)
{
    std::vector<int> a{1, 2, 3};
    std::vector<int> b{4, 5, 6};
    auto             z = std::ref(a) | radr::zip_with(std::ref(b));
...
TEST(zip_with, ConceptsForBidirectionalRanges)
{
...
TEST(zip_with, ConceptsForForwardRanges)
{
...
TEST(zip_with, ConceptsMixedCategories)
{
    std::vector<int> vec{1, 2, 3};
...
TEST(zip_with, ConceptsForRandomAccessRangesOwning)
{
    std::vector<int> a{1, 2, 3};
    std::vector<int> b{4, 5, 6};
...
TEST(zip_with, ConceptsForBidirectionalRangesOwning)
{
    std::list<int> a{1, 2, 3};
    std::list<int> b{4, 5, 6};
    auto           z = std::move(a) | radr::zip_with(std::ref(b));
...
TEST(zip_with, ConceptsForForwardRangesOwning)
{
    std::forward_list<int> a{1, 2, 3};
    std::forward_list<int> b{4, 5, 6};
    auto                   z = std::move(a) | radr::zip_with(std::ref(b));
...
TEST(zip_with, RandomAccessIndexing)
{
    std::vector<int>  a{1, 2, 3, 4, 5};
...
TEST(zip_with, IteratorDistance)
{
    std::vector<int> a{1, 2, 3, 4};
...
TEST(zip_with, IteratorIncrement)
{
    std::vector<int> a{1, 2, 3};
    std::vector<int> b{10, 20, 30};
    auto             z = std::ref(a) | radr::zip_with(std::ref(b));
...
TEST(zip_with, IteratorComparison)
{
    std::vector<int> a{1, 2, 3};
    std::vector<int> b{10, 20, 30};
    auto             z = std::ref(a) | radr::zip_with(std::ref(b));
...
TEST(zip_with, CommonRangeWithCommonInputs)
{
    std::vector<int> a{1, 2, 3};
    std::vector<int> b{4, 5, 6};
    auto             z = std::ref(a) | radr::zip_with(std::ref(b));
...
TEST(zip_with, owning_copy_test)
{
    ...
    using T = decltype(std::vector{1, 2, 3, 4, 5, 6} | radr::zip_with(radr::iota(0)));
...
TEST(zip_with_sp, SingleRange)
{
    std::vector<int> vec{1, 2, 3};
    auto             z = std::move(vec) | radr::to_single_pass | radr::zip_with();
...
TEST(zip_with_sp, ThreeRanges)
{
    ...
    std::vector<char>   b{'x', 'y', 'z'};
    std::vector<double> c{1.1, 2.2, 3.3};
    auto                z = std::move(a) | radr::to_single_pass | radr::zip_with(std::ref(b), std::ref(c));
...
TEST(zip_with_sp, MixedSinglePassAndMultiPass)
{
    ...
    std::vector<char> chars{'a', 'b', 'c'};
    auto              z = std::move(iota_gen) | radr::zip_with(std::ref(chars));
...
TEST(zip_with_sp, ReferenceTypes)
{
    std::vector<int>         xs{1, 2, 3};
    std::vector<std::string> ys{"a", "b", "c"};
...
TEST(zip_with_sp, ReferenceTypes2)
{
    std::vector<int> xs{1, 2, 3};
...
TEST(zip_with_sp, StopsAtShortest)
{
