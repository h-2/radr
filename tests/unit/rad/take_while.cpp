...
struct take_while_forward : public testing::Test
{
    ...
    static void type_checks_impl()
    {
        ...
        EXPECT_EQ(radr::constant_range<in_t>, radr::constant_range<container_t>);
        EXPECT_EQ(radr::const_symmetric_range<in_t>, radr::const_symmetric_range<container_t>);
...
TYPED_TEST(take_while_forward, chained_adaptor)
{
    ...
    using cit_t       = radr::detail::
      take_while_iterator<radr::const_iterator_t<container_t>, radr::const_iterator_t<container_t>, chained_pred_t>;
...
TEST(take_while_forward_, noncommon)
{
    ...
    using cit_t =
      radr::detail::take_while_iterator<radr::const_iterator_t<container_t>, radr::const_sentinel_t<container_t>, pred_t>;
    using csen_t = std::default_sentinel_t;
...
TEST(take_while_forward_, noncommon2common)
{
    /* construct container */
    ...
    {
        auto ra = std::ref(in) | radr::take_while(fn) | radr::to_common;
        auto e  = radr::begin(ra);
        while (e != radr::end(ra))
            ++e;
...
TEST(take_while, owning_copy_test)
{
