...
public:
    ...
    constexpr explicit take_while_sentinel(Sen end, Func func) : end_{std::move(end)}, func_{std::move(func)} {}
    ...
    constexpr take_while_sentinel(take_while_sentinel<OtherIter, OtherSen, Func> s) : end_{std::move(s.end_)}, func_{std::move(s.func_)}
    {}
    ...
    {
        return lhs == rhs.end_ || !invoke(*rhs.pred_, *lhs);
    }
};

inline constexpr auto take_while_borrow =
  []<std::ranges::forward_range URange, typename Fn>(URange && urange, Fn fn)
{
    static_assert(borrowed_mp_range<URange>, "The constraints for radr::take_while's underlying range are not satisfied.");
    static_assert(filter_func_constraints<Fn, iterator_t<URange>> && filter_func_constraints<Fn, const_iterator_t<URange>>,
                  "The constraints for radr::take_while's functor are not satisfied.");

    constexpr auto generic_impl =
  []<typename UIt, typename USen,typename UCIt, typename UCSen, typename Fn_>(UIt it, USen sen, UCIt, UCSen, Fn_ _fn)
    {
        using It  = UIt;
    ...
    constexpr auto folding_impl = []<typename UIt, typename USen, typename UCIt, typename UCSen, typename UFn, typename Fn_>(UIt it, take_while_sentinel<UIt, USen, UFn> sen, UCIt, take_while_sentinel<UCIt, UCSen, UFn>, Fn_ _fn)
    {
    ...
    return overloaded{
        generic_impl, folding_impl
      }(radr::begin(urange), radr::end(urange),radr::cbegin(urange), radr::cend(urange), std::move(fn));
};

inline constexpr auto take_while_coro =
  []<std::ranges::input_range URange, typename Fn>(
    URange && urange,
    Fn        fn)
{
    ...
    static_assert(weak_indirectly_unary_invocable<Fn, std::ranges::iterator_t<URange>>,
                  "The constraints for radr::take_while's functor are not satisfied.");
 ...
 *   * `std::is_object_v<Predicate>`
 *   * `std::indirect_unary_predicate<Predicate, radr::iterator_t<URange>>`
 *   * `std::indirect_unary_predicate<Predicate, radr::const_iterator_t<URange>>`
 *
 ...
 *   * `std::is_object_v<Predicate>`
 *   * `radr::weak_indirectly_unary_invocable<Predicate, radr::iterator_t<URange>>`
 *
 */

inline constexpr auto take = detail::pipe_with_args_fn{detail::take_while_coro, detail::take_while_borrow};
} // namespace cpo
} // namespace radr
